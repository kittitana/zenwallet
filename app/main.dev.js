...
const setupAutoUpdate = () => {
  ...
  autoUpdater.on(APP_CHECK_FOR_UPDATES, () => {
    mainWindow.webContents.send(APP_CHECKING_FOR_UPDATES)
  })

  autoUpdater.on(APP_UPDATE_AVAILABLE, (event, info) => {
    mainWindow.webContents.send(APP_UPDATE_AVAILABLE, event, info)
  ...
  autoUpdater.on(APP_UPDATE_ERROR, error => {
    mainWindow.webContents.send(APP_UPDATE_ERROR, error)
  })

  autoUpdater.on(APP_UPDATE_DOWNLOADED, (event, info) => {
    mainWindow.webContents.send(APP_UPDATE_DOWNLOADED, event, info)
